...
class Game extends React.PureComponent {
  ...
  onCellClick = cell => {
    this.setState(prevState => {
      ...
      let {score, wrongsLeft} = prevState;
      ...
      return {
        ...
        wrongsLeft: newWrongsLeft ? newWrongsLeft : wrongsLeft
      };
    });
  }
  render() {
    return (
      <div className="game">
        <div className="help">You will have 3 seconds to memorize {this.state.challengeCells.length === 0 ? "X" : this.state.challengeCells.length} blue random cells</div>
        <p className="score">Score: {this.state.score}</p>
        <div className="grid">{this.state.grid.map(cell => {
            ...
            return <Cell id={cell} onClick={this.onCellClick} key={cell} showAsSelected={this.showSelectedCells() && cellIsSelected} showAsChallenge={this.showChallengeCells() && cellIsChallenge} cellIsChallenge={cellIsChallenge} cellWidth={this.state.cellWidth} /> })}
           </div>
           {this.gameIsIdle() && (
